...
class operaciones
{
  ...
  function sumarMesAsignadoDiaPago($cantidadSumada,$diapago){
    $dia=$diapago;
    if (2 <= $dia && $dia <= 10) {
      $dia=10;
    }elseif (11 <=$dia && $dia <= 20) {
     ...
     if ($rango==1) {
       for ($i=0; $i < count($paquetes); $i++) {
         ...
         if ($paquete['nivel']==1) {
           ...
           }elseif($paquete['tipo']==2){
             $descripcion="Bono de afiliacion por Renovación nivel ".$paquete['nivel']." (".$paquete['nombres']. " " . $paquete['apellidos']." )";
             $comision=$paquete['paquete']*0.04;
           }
